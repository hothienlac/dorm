...
import { RolesEnum } from '@dorm/models';
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (endpointRoles.includes(RolesEnum.SELF)) {
      if (token.preferred_username === this.resolveUsername(request)) {
        ...
        throw new ForbiddenException(`SELF and Parent of SELF only`);
      }
    }
    ...
    if (endpointRoles.includes(RolesEnum.ADMIN)) {
      if (this.isRoleOverlay(token.realm_access.roles, [RolesEnum.ADMIN])) {
        ...
        throw new ForbiddenException(`Dormitory admin users only`);
      }
    }
    ...
    //     return true;
    //   } else {
    //     throw new ForbiddenException(`Dormitory users only`);
    //   }
    // }
